...
if (!current_user_can('manage_options')) wp_send_json_error('Permission', 403);
check_ajax_referer('cgsd_admin', 'nonce');
...
if ($sheet_id) update_option('cgsd_sheet_id', $sheet_id);
if ($range) update_option('cgsd_range', $range);
if ($api_key) update_option('cgsd_api_key', $api_key);
...
if (empty($data['values']) || count($data['values']) < 2) { wp_send_json_error('No data found', 400); } //
  สร้างตารางหลัก (ถ้ายังไม่มี) $charset=$wpdb->get_charset_collate();
